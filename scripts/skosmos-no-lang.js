  ...
  $(skosmosInputSelector).each(function () {
    ...
    if (!skosmosInput.hasAttribute('data-skosmos')) {
      ...
      if ($("[data-cvoc-parentfield='" + parentField + "']").length > 0) {
        ...
        anchorSib = $(skosmosInput)
          .parentsUntil("[data-cvoc-parentfield='" + parentField + "']")
          .last();
      }
      ...
      $(anchorSib)
        .parent()
        .find('.cvoc-vocab')
        .append('<select id=' + vocabId + ' class="form-control add-resource select2" tabindex="-1" aria-hidden="true">');
      ...
      if ($(anchorSib).parent().find('.cvoc-vocab').length != 0) {
        $(anchorSib)
          ...
          .after(
            $('<div/>')
              .addClass('cvoc-term col-sm-9')
              .append($('<label/>').text('Term'))
              .append('<select id=' + selectId + ' class="form-control add-resource select2" tabindex="-1" aria-hidden="true">')
          );
        $('#' + selectId).attr('data-cvoc-cur-vocab', vocab);
        if (Object.keys(vocabs).length == 1 || $(skosmosInput).val().startsWith('http')) {
        ...
        if ($("[data-cvoc-parentfield='" + parentField + "']").length > 0) {
          ...
          for (var key in managedFields) {
            if (key == 'vocabularyName') {
              $(parent)
                .find("input[data-cvoc-managed-field='" + managedFields[key] + "']")
                .attr('value', $('#' + selectId).attr('data-cvoc-cur-vocab'));
            } else if (key == 'vocabularyUri') {
              $(parent)
                .find("input[data-cvoc-managed-field='" + managedFields[key] + "']")
                .attr('value', vocabs[$('#' + selectId).attr('data-cvoc-cur-vocab')].vocabularyUri);
