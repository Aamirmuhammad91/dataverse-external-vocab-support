...
function updateSkosmosInputs() {
  ...
  $(skosmosInputSelector).each(function () {
    ...
    if (!skosmosInput.hasAttribute('data-skosmos')) {
      ...
      var ontology = $(anchorSib)
        .parent()
        .children()
        .find("input[data-cvoc-managed-field='" + managedFields['vocabularyName'] + "']")
        .attr('value');
            ...
            if (key == 'vocabularyName') {
              $.ajax({
                ...
                success: function (onthology, status) {
                  $(parent)
                    .find("input[data-cvoc-managed-field='" + managedFields['vocabularyName'] + "']")
                    .attr('value', onthology.acronym);
                },
                failure: function (jqXHR, textStatus, errorThrown) {
                  $(parent)
                    .find("input[data-cvoc-managed-field='" + managedFields['vocabularyName'] + "']")
                    .attr('value', data.voc.substring(data.voc.lastIndexOf('/') + 1));
                },
              });
            } else if (key == 'vocabularyUri') {
              $(parent)
                .find("input[data-cvoc-managed-field='" + managedFields['vocabularyUri'] + "']")
                .attr('value', data.voc.replace(cvocUrl, cvocUiUrl));
            } else if (key == 'termName') {
              $(parent)
                .find("input[data-cvoc-managed-field='" + managedFields['termName'] + "']")
