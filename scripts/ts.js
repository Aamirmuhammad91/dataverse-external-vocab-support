...
function expandPeople() {
  ...
  $(personSelector).each(function () {
    ...
    if (!$(personElement).hasClass('expanded')) {
      ...
      if (id.startsWith('https://service.tib.eu/ts4tib/api/ontologies/')) {
        id = id.substring(18);
      }
      ...
      $.ajax({
        type: 'GET',
        url: 'https://service.tib.eu/ts4tib/api/ontologies/' + 'cgo' + '/terms?iri=' + encodeURIComponent('https://www.tno.nl/agrifood/ontology/common-greenhouse-ontology#Plant'),
        dataType: 'json',
        headers: {
        ...
        success: function (person, status) {
          ...
          var name = person['_embedded']['terms']['label'] + ', ' + person['_embedded']['terms']['iri'];
          var html =
            "<a href='https://terminology.tib.eu/ts/ontologies/" +
            'cgo' +
            '/terms?iri=' +
            encodeURIComponent('https://www.tno.nl/agrifood/ontology/common-greenhouse-ontology#Plant') +
...
function updatePeopleInputs() {
  ...
  $(personInputSelector).each(function () {
    ...
    if (!personInput.hasAttribute('data-person')) {
          ...
          processResults: function (data, page) {
            return {
              results: data['response']['docs']
                ...
                .sort((a, b) => (localStorage.getItem(b['ontology_name']) ? 1 : 0))
                .map(function (x) {
                  return {
                    text: x['label'] + ' ' + x['ontology_name'] + ', ' + x['short_form'],
                    id: x['ontology_name'],
                    iri: x['iri'],
      ...
      if (id.startsWith('https://terminology.tib.eu/ts/ontologies/')) {
        id = id.substring(18);
        $.ajax({
          type: 'GET',
          url: 'https://service.tib.eu/ts4tib/api/ontologies/' + 'cgo' + '/terms?iri=' + encodeURIComponent('https://www.tno.nl/agrifood/ontology/common-greenhouse-ontology#Plant'),
